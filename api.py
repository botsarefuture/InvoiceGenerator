...
class QrCodeBuilder(object):
    ...
    def _fill(self, invoice):
        qr_kwargs = {
            'beneficiary': "LuovaClub",
            'iban': "FI1313753500061584",
            'amount': invoice.use_tax and invoice.price_tax or invoice.price,
            'remittance': "MTKK" + str(invoice.provider.bank_code.replace("-", "")),
        }
        
        print(qr_kwargs)

        if invoice.variable_symbol:
            qr_kwargs['reference'] = invoice.variable_symbol
        ...
        epc_qr = consumer_epc_qr(**qr_kwargs)
        return epc_qr
    ...
    def filename(self):
        ...
        img = qrcode.make(self.qr.to_str())
