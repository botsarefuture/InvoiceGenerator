...
class SimpleInvoice(BaseInvoice):
    ...
    def _add_item(self, xml_invoice, item):
        ...
        ET.SubElement(invoice_item, "{%s}quantity" % self._inv_ns).text = str(
            item.count
        )
        ...
        ET.SubElement(home_currency, "{%s}unitPrice" % self._typ_ns).text = str(
            item.price
        )

        ET.SubElement(invoice_item, "{%s}text" % self._inv_ns).text = str(
            item.description
        )[:90]
        if item.tax in self.inv_tax_rates:
            ET.SubElement(invoice_item, "{%s}rateVAT" % self._inv_ns).text = (
                self.inv_tax_rates[item.tax]
            )
        else:
            logger.warning(
                "Tax rate %s is not among the tax rates accepted by Pohoda system"
                % item.tax
            )
    ...
    def add_elements(self, parrent_element, namespace, element_map):
        for element, content in element_map.items():
            if content is not None:
                ...
                ET.SubElement(parrent_element, "{%s}%s" % (namespace, element)).text = (
                    str(content)
                )
    ...
    def _invoice_header(self, invoice_header):
        ET.SubElement(invoice_header, "{%s}invoiceType" % self._inv_ns).text = (
            "issuedInvoice"
        )
        number = ET.SubElement(invoice_header, "{%s}number" % self._inv_ns)
        self.add_elements(
            number, self._typ_ns, {"numberRequested": self.invoice.number}
        )
        ...
        payment_account = ET.SubElement(
            invoice_header, "{%s}paymentAccount" % self._inv_ns
        )
        account_element_map = {
        ...
        partner_identity = ET.SubElement(
            invoice_header, "{%s}partnerIdentity" % self._inv_ns
        )
    ...
    def _invoice_summary(self, invoice_summary):
        ET.SubElement(invoice_summary, "{%s}roundingDocument" % self._inv_ns).text = (
            "math2one"
        )
        home_currency = ET.SubElement(
            invoice_summary, "{%s}homeCurrency" % self._inv_ns
        )
        ...
        for rate_ident, rate in self.tax_rates.items():
            if rate in breakdown:
                ...
                ET.SubElement(
                    home_currency, "{%s}price%s" % (self._typ_ns, rate_camel)
                ).text = str(breakdown[rate]["total_tax"])
                if rate_ident != "none":
                    ET.SubElement(
    ...
    def gen(self, filename):
        ...
        data_pack_item = ET.SubElement(
            data_pack,
            "{%s}dataPackItem" % self._dat_ns,
            version="2.0",
            id=self.invoice.number,
        )
        xml_invoice = ET.SubElement(
            data_pack_item, "{%s}invoice" % self._inv_ns, version="2.0"
        )
        ...
        invoice_summary = ET.SubElement(
            xml_invoice, "{%s}invoiceSummary" % self._inv_ns
        )
